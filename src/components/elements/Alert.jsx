...
const Alert = ({ type, message }) => {
  ...
  return (
    <BootstrapAlert variant={`${alertType} d-flex align-items-center`} role="alert">
      {alertType === "success" && <SuccessIcon />}
      {(alertType === "primary" || alertType === "info") && <InfoIcon />}
      {(alertType === "warning" || alertType === "danger") && <WarningIcon />}
      <div>{message}</div>
    </BootstrapAlert>
  );
};
